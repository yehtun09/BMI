...
@section('content')
    <div class="card">
        ...
        <div class="card-body mt-4">
            <form method="POST" action="{{ route('admin.status-all.update',[$status->id]) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="form-group col-5">
                        ...
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                            name="name" id="name" value="{{ old('name', $status->name) }}">
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group col-5">
                        ...
                        <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text"
                            name="type" id="type" value="{{ old('type', $status->type ) }}">
                        @if ($errors->has('type'))
                            <div class="invalid-feedback">
                                {{ $errors->first('type') }}
                            </div>
