...
@section('content')
    <div class="card">
        <h5 class="card-header font-weight-bold mb-4"> {{ trans('global.edit') }} {{ trans('cruds.seller.title_singular') }}
        </h5>
        <div class="card-body">
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12">
                        <div class="form-group">
                            <label class="required"
                                for="seller_type_id">{{ trans('cruds.seller.fields.seller_type_id') }}</label>
                            <select name="seller_type_id" id="" class="select2 {{$errors->has('seller_type_id') ? 'is-invalid' : ''}}">
                                <option value="">-- Select Buyer Category --</option>
                                @foreach ($seller_types as $key => $seller_type)
                                    <option value="{{ $seller_type->id }}"
                                        {{ $seller_type->id == old('seller_type_id') ? 'selected' : '' }}> {{ $seller_type->name }}
                                    </option>
                                @endforeach
                            </select>
                            <span class="seller_type_id"></span>
