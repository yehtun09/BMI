...
@section('content')
    <div class="card">
        <h5 class="card-header font-weight-bold">{{ trans('global.update') }} {{ trans('cruds.sellerUserStatus.title_singular') }}</h5>
        <div class="card-body mt-4">
            <form method="POST" action="{{ route('admin.seller-user-statuses.update',[$sellerUserStatus->id]) }}" id="sellerUserStatus">
                @csrf
                @method('PUT')
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('seller_product_id') ? 'is-invalid' : '' }}" name="seller_product_id" id="seller_product_id">
                                ...
                                @foreach($sellerProducts as $id => $sellerProduct)
                                    <option value="{{ $sellerProduct->id }}" {{ old('seller_product_id' , $sellerUserStatus->seller_product_id ) == $sellerProduct->id ? 'selected' : '' }}>{{ $sellerProduct->sellerProductType->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('user_id') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                                ...
                                @foreach($users as $id => $user)
                                    <option value="{{ $id }}" {{ old('user_id',$sellerUserStatus->user_id) == $id ? 'selected' : '' }}>{{ $user }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('status_id') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                                ...
                                @foreach($statuses as $id => $status)
                                    <option value="{{ $id }}" {{ old('status_id',$sellerUserStatus->status_id) == $id ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12">
                        <div class="form-group">
                            ...
                            <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}"  type="datetime-local" name="date" id="date" value="{{ old('date' ,$sellerUserStatus->date) }}">
